...
def start_game(chat_id, level):
    ...
    # ----------------------------------------------
    # از اینجا بازی فعال شده است.
    # هنوز دکمه توقف را ارسال نمی‌کنیم؛
    # فقط شمارش معکوس اجرا می‌شود.
    # ----------------------------------------------
    ...
    # ----------------------------------------------
    # شروع واقعی بازی
    # از این لحظه دکمه توقف نمایش داده می‌شود.
    # ----------------------------------------------
...
def handle_update(update):
    ...
    if message:
        ...
        if text == "شروع چالش":
            ...
            # پیام انتظار برای بیدار شدن سرور
            send_message(
                chat_id,
                (
                    "🚀 <b>شروع چالش</b>\n\n"
                    "⏳ تا راه‌اندازی بات کمی تأمل کنید..."
                )
            )

            # فقط انتخاب سطح نمایش داده می‌شود.
            # هنوز بازی شروع نشده است.
